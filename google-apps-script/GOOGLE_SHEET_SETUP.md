...
2. Title the form: **25th Jharkhand State Pedicon & 29th Annual Conference IAP Jamshedpur**.
3. Add the following questions (set them as **Required**):
   - **Full Name** (Short Answer)
   - **Email** (Short Answer - set validation to "Email Address")
   - **Phone Number** (Short Answer)
   - **College / Hospital** (Short Answer)
   - **Profession** (Dropdown or Multiple Choice: *Delegate / PG Student / Accompanying Person / Senior Citizen*)
   - **City** (Short Answer)
   - **Payment Reference / Transaction ID** (Short Answer, optional but recommended)
   - **Upload Payment Screenshot** (File Upload, optional)
...
4. Copy the complete code from [google-apps-script.js](file:///e:/COdeNixxx/Confrence/google-apps-script/google-apps-script.js) and paste it into the editor.
5. In the script, locate the `ORGANIZER_EMAIL` variable on line 20 and replace it with your official registrar email (e.g., `dr.sunilsingh@example.com` or `iapjamshedpur@gmail.com`).
...
2. Fill in the fields with your own details:
   ...
   - City: *Jamshedpur*
3. Click **Submit**.
4. Check your inbox! You should receive a beautifully styled HTML email confirming your registration, containing the event venue details and a link to join the WhatsApp announcements group.
5. The organizer's email (`iapjamshedpur@gmail.com`) will also receive an administrative notification containing a summary of the participant's entries.

### Method B: Manual Test Run (Inside Apps Script Editor)
Since we've added test simulation fallbacks directly in [google-apps-script.js](file:///e:/COdeNixxx/Confrence/google-apps-script/google-apps-script.js), you can test the script execution directly:
1. In the Apps Script Editor, select the function `onFormSubmit` from the dropdown next to the **Run** and **Debug** buttons.
2. Click **Run**.
3. The script will automatically trigger manual simulation mode, use mock placeholders (such as Name: "John Doe", Email: "iapjamshedpur@gmail.com"), send verification emails, and complete the execution.
